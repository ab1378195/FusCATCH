...
class MemEntropyLoss(torch.nn.Module):
    def __init__(self, eps = 1e-12):
        super(MemEntropyLoss, self).__init__()
        self.eps = eps

    def forward(self, input): # input: (bs*patch_num, nvars, mem_dim)
        bp, nvars, mem_dim = input.shape
        att = input.view(-1, mem_dim)
        entropy = att * torch.log(att + self.eps)
